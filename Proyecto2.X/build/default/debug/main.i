...
void main(void) {
    ...
    while(1){
        ...
        if(a == 2){
            PORTBbits.RB4 = 1;
            PORTBbits.RB3 = 0;
            mandar("Presione 1 para controlar la garra");
            mandar("Presione 2 para controlar el brazo");
            while(RCIF == 0){;}
            _delay((unsigned long)((100)*(500000/4000000.0)));
            recibido = RCREG;
            if(RCREG == 1){
            mandar("Solo debe ingresar valores entre 7 y 16");
            while(RCREG >= 7 && RCREG <= 16){
                CCPR1L = RCREG;
                }
            }else if(RCREG == 2){
                mandar("Solo debe ingresar valores entre 7 y 16");
